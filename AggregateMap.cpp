...
//	
	...
	desc.add_options()
		...
		("output,o", prog_opt::value<std::string>(&output_file)->default_value("aggregated-map.tif"), "path where the output gtif raster is saved")
	...
	Map_Double_SPtr output_map(new Map_Double(template_rows, template_cols, out_map_no_data_val));
	output_map->SetNoDataValue(out_map_no_data_val);
	...
	for (unsigned int i = 0; i < template_rows; ++i)
	{
		for (unsigned int j = 0; j < template_cols; ++j)
		{
			if (template_m->Get(i, j) != template_no_data)
			{
				...
				if (do_max) output_map->Get(i, j) = max;
				else output_map->Get(i, j) = mean;
				if (no_data_count != 0)
				{
					if (((count / no_data_count) < 2) && majority)
					{
						output_map->Get(i, j) = out_map_no_data_val;
					}
				}
			}
			else
	...
	write_map(output_path, GDT_Float64, output_map, template_WKT_projection, template_transform, driverName);
